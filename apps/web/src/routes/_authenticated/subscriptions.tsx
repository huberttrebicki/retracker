...
function SubscriptionsPage() {
	...
	async function handleStatusChange(sub: Subscription, newStatus: Status) {
		...
		if (newStatus === "cancelled") {
			const calculatedEnd = getNextPayment(
				sub.startedAt,
				sub.intervalCount,
				sub.interval,
			);
													...
													<div className="mt-3 flex justify-end gap-1">
														<DropdownMenu>
															...
															>
																...
																{
																	statuses.find((s) => s.value === sub.status)
																		?.label
																}
															</DropdownMenuTrigger>
																...
																{statuses
																	...
																	.map((s) => (
																		<DropdownMenuItem
																			key={s.value}
																			onClick={() =>
																				handleStatusChange(sub, s.value)
																			}
																		>
																			{s.value === "active" && (
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Your subscription to "{cancellingSubscription?.name}" will expire
							on:
						</DialogDescription>
					</DialogHeader>
					<Input
					...
					/>
					<DialogFooter>
						<Button variant="outline" onClick={() => confirmCancellation(null)}>
