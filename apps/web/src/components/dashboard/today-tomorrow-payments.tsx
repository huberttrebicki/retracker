...
function PaymentList({ label, payments, currency, convert }: { label: string; payments: Subscription[]; currency: string; convert: (amount: number, from: string) => number }) {
  const total = payments.reduce((sum, s) => sum + convert(Number(s.price), s.currency), 0)

  return (
    <div className="flex-1 min-w-0">
      <div className="mb-3 flex items-baseline justify-between">
        <p className="text-sm font-semibold text-purple-700 dark:text-purple-400">{label}</p>
        {payments.length > 0 && (
        ...
        <div className="flex flex-col gap-2">
          {payments.map((sub) => (
            <div key={sub.id} className="flex items-center gap-2.5">
              {sub.provider.logo ? (
                <img src={sub.provider.logo} alt={sub.provider.name} className="size-7 shrink-0 rounded object-contain" />
              ) : (
  ...
  return (
    <Card className="border-t-2 border-t-purple-400 dark:border-t-purple-600">
      <CardContent className="flex gap-6 p-5">
        <PaymentList label="Today" payments={todayPayments} currency={currency} convert={convert} />
        <div className="w-px bg-border" />
        <PaymentList label="Tomorrow" payments={tomorrowPayments} currency={currency} convert={convert} />
      </CardContent>
    </Card>
  )
}
